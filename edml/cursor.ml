...
let make () = { col = 0; row = 0; real_col = 0; offset_row = 0 }

let adjust_column cursor line _vp_dimensions =
...
let adjust_row cursor vp_dimensions =
  ...
  if current_row - cursor.offset_row > vp_height - 1
...
let move_right cursor text_object _vp_dimensions =
  let open Text_object in
  let line = List.nth_exn text_object.content cursor.row in
  let cursor =
