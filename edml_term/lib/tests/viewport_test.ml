...
let%test "should get correct diffs" =
  let sample_cell = { symbol = 'x'; styles = Load_colorscheme.empty_style () } in
  let expected =
...
let%test "should make new vp with correct text" =
  let base_style = Load_colorscheme.empty_style () in
  let expect =
    { cells =
...
let%test "should make new vp with correct cell" =
  ...
  set_cell 'x' ~col:0 ~row:1 ~vp ~styles:base_style;
  set_cell 'x' ~col:1 ~row:1 ~vp ~styles:base_style;
  [%eq: t] !vp expect
;;
