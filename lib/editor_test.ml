...
let%test "fill viewport respects position" =
  ...
  let rec make_cells acc n =
    if n <= 0 then acc else make_cells (make_cell () :: acc) (n - 1)
  in
  let rows = 3 in
  let cols = 8 in
  let cells = make_cells [] (rows * cols) in
...
let%test "should render correct ratios" =
  ...
  let pane_a = Pane.make 0 in
  let pane_b = Pane.make 0 in
  let pane_c = Pane.make 0 in
  let branch =
  ...
  in
  let tab = { panes = Branch branch } in
  let editor =
